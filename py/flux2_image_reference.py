...
class NakuNode_Flux2:
    ...
    def apply_reference(self, vae, strength, image1=None, image2=None, image3=None, image4=None, image5=None, positive_conditioning=None, negative_conditioning=None):
        # 收集图像
        ...
        if positive_conditioning is not None:
            ...
            processed_positive_conditioning = self.inject_features(positive_conditioning, reference_latents, strength)
        else:
            # 如果没有原始conditioning，返回基于参考latent的新conditioning
            processed_positive_conditioning = self.create_conditioning_from_latents(reference_latents, strength)

        # 处理negative conditioning
        if negative_conditioning is not None:
            # 对negative conditioning也应用相同的处理，但可能使用不同的强度或其他调整
            processed_negative_conditioning = self.inject_features(negative_conditioning, reference_latents, strength)
        else:
            # 如果没有negative conditioning，返回None或基于参考latent的conditioning
            processed_negative_conditioning = self.create_conditioning_from_latents(reference_latents, 0.0) if len(reference_latents) > 0 else None

        return (processed_positive_conditioning, processed_negative_conditioning)
