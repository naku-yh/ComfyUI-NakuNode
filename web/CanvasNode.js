    ...
    initializeControls() {
        ...
        const resetButton = this.createButton('重置', () => {
            if (this.node.id) {
                ...
                // 创建新节点
                let new_node = LiteGraph.createNode("NakuNode_画布");
...
app.registerExtension({
    name: "Custom.NakuNode_画布",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name === "NakuNode_画布") {           
...
app.registerExtension({
    name: "Custom.NakuNode_画布工具",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name === "NakuNode_画布工具") {
            // 处理连接变化
