...
class QueueManager {
  constructor() {
    ...
    this.isNakuTriggered = false; // 标记是否由 NakuNodes 扩展触发
  ...
  initializeHooks() {
    ...
    api.queuePrompt = async function(index, prompt, ...args) {
      // 仅在 NakuNodes 扩展触发时才修改 prompt.output
      if (this.isNakuTriggered && this.queueNodeIds && this.queueNodeIds.length && prompt.output) {
  ...
  async queueOutputNodes(nodeIds) {
    try {
      ...
      this.isNakuTriggered = true; // 设置 NakuNodes 触发标记
      await app.queuePrompt();
    } catch (e) {
      console.error("队列节点时出错:", e);
    } finally {
      this.queueNodeIds = null;
      this.isNakuTriggered = false; // 清除 NakuNodes 触发标记
    }
  }
  getLastMouseEvent() {
...
function queueSelectedOutputNodes() {
  ...
  if (!selectedNodes || Object.keys(selectedNodes).length === 0) {
    console.log("[NakuNodes]队列: 没有选中的节点");
    return;
  }

  const outputNodes = getOutputNodes(Object.values(selectedNodes));
  if (!outputNodes || outputNodes.length === 0) {
    console.log("[NakuNodes]队列: 选中的节点中没有输出节点");
    return;
  }

  console.log(`[NakuNodes]队列: 执行 ${outputNodes.length} 个输出节点`);
...
app.registerExtension({
  name: "NakuNodes.QueueNodes",
  commands: [
    {
      id: "NakuNodes.QueueSelectedOutputNodes",
      icon: "pi pi-play",
      label: "执行选中的输出节点",
      function: queueSelectedOutputNodes
    },
    {
      id: "NakuNodes.QueueGroupOutputNodes",
      icon: "pi pi-sitemap",
      label: "执行组内输出节点",
      function: queueGroupOutputNodes
    }
