...
app.registerExtension({
    name: "Comfy.NakuNode_图像标注节点V2",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "NakuNode_图像标注节点V2") {
...
// NAKU Video Save Extension
import { api } from "../../../scripts/api.js";

app.registerExtension({
    name: "Naku.VideoSave",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "NakuNode_VideoSave") {
            ...
            function getHeaderAndWidgetHeight(node) {
                ...
                if (node.widgets) {
                    for (const w of node.widgets) {
                        if (w.name !== "naku_preview" && w.type !== "hidden") {
            ...
            nodeType.prototype.getExtraMenuOptions = function (_, options) {
                ...
                newOptions.push({
                    ...
                    callback: () => {
                        const params = this.properties["naku_output_cache"];
                        if (params) {
            ...
            nodeType.prototype.onResize = function (size) {
                if (this.naku_aspect) {
                    ...
                    const targetVideoHeight = size[0] / this.naku_aspect;
                ...
                const widget = this.widgets?.find(w => w.name === "naku_preview");
                if (widget?.element) {
                    // Ensure DOM element covers the remaining area perfectly
                    widget.element.style.width = `${size[0]}px`;
            ...
            nodeType.prototype.onExecuted = function (message) {
                if (message?.naku_output) {
                    this.properties["naku_output_cache"] = message.naku_output[0];
                    updateVideoPreview(this, message.naku_output[0]);
                }
            };

            nodeType.prototype.onConfigure = function () {
                if (this.properties?.["naku_output_cache"]) {
                    updateVideoPreview(this, this.properties["naku_output_cache"]);
                }
            };

            // --- 5. Video UI Implementation ---
            function updateVideoPreview(node, data) {
                let widget = node.widgets?.find(w => w.name === "naku_preview");
                ...
                if (!widget) {
                    ...
                    widget = node.addDOMWidget("naku_preview", "preview", element, {
                ...
                video.onloadedmetadata = () => {
                    if (video.videoWidth && video.videoHeight) {
                        node.naku_aspect = video.videoWidth / video.videoHeight;
...
app.registerExtension({
    name: "Comfy.NakuNode_简易画板",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "NakuNode_简易画板") {
